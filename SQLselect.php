<?php
    ...
    //include an external PHP file into this file
        ...
        //1.
        require 'dbConnect.php';        //copies content of dbConnect.php INTO this page
       ...
       $stmt->setFetchMode(PDO::FETCH_ASSOC);       //setting ALL fetch commands to return associative array
        ...
        //fetch is 1 row, always fetches the next row
        //fetch all is all of the rows
       /*
            $row = $stmt->fetch();       //should get first row from result within statement       $row is associative array
            echo $row["events_name"];

            $row = $stmt->fetch();       //should get second row from result within statement
            echo $row["events_name"];

            $row = $stmt->fetch();       //should get third row from result within statement
            echo $row["events_name"];

            $row = $stmt->fetch();       //should get fourth row from result within statement
            echo $row["events_name"];
        */
    //can put these ^ in a loop - loop is in the body - cannot have both loop and these^^^^will only cyle through once, need to create another fetch if you want to run it again
